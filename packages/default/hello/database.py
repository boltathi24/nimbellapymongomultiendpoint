...
class PyMongo:
    ...
    def getData(table, key, value):
        cursor = PyMongo.getConn()["NewDB"][table].find({key: value}, {'_id': False})
        return loads(dumps(cursor))

    @staticmethod
    def getAllData( table):
        cursor = PyMongo.getConn()["NewDB"][table].find({}, {'_id': False})
        return loads(dumps(cursor))

    @staticmethod
    def insertData( table, data):
        PyMongo.getConn()["NewDB"][table].insert(data)

    @staticmethod
    def updateData( table, keyToUpdate, valueToUpdate, searchKey, searchValue, key2, value2):
        PyMongo.getConn()["NewDB"][table].update({searchKey:searchValue},{"$set":{keyToUpdate:valueToUpdate,key2:value2}},upsert = True)
